...
def parse_args(args=None):
    ...
    parser.add_argument('-t', '--type_dim', default=500, type=int)
...
def e2t_save_model(model, optimizer, save_variable_list, args):
    ...
    torch.save({
        **save_variable_list,
        'e2t_model_state_dict': model.state_dict(),
...
def main(args):
    ...
    if args.do_train:
        ...
        #Training Loop
        num_epoch = int(args.max_steps/5000)
        
        for epoch in range(num_epoch):
            logging.info('Starting epoch ' + str(epoch))
            logging.info('Training RotatE epoch'+str(epoch))
            for step in range(init_step+epoch*5000, init_step+(epoch+1)*5000):
            ...
            e2t_model.entity_embedding.data = kge_model.entity_embedding.data
            for step in range(init_step+epoch*5000, init_step+(epoch+1)*5000):
                # e2t_model.entity_embedding.from_pretrained(weight)
                ...
                if (step+1) % args.save_checkpoint_steps == 0:
                    ...
                    e2t_save_model(e2t_model, e2t_optimizer, save_variable_list, args)
                ...
                if args.do_valid and (step+1) % (args.valid_steps/2) == 0:
                    logging.info('Evaluating on Valid Dataset...')
    ...
    if args.do_valid:
        logging.info('Evaluating on E2T Valid Dataset...')
        metrics = e2t_model.test_step(e2t_model, valid_pairs, all_true_pairs, args)
        log_metrics('Valid', step, metrics)
    
    if args.do_test:
        logging.info('Evaluating on E2T Test Dataset...')
